      ...
      <div className="relative w-full sm:max-w-lg bg-slate-900 border border-slate-800 rounded-t-2xl sm:rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-[11px] text-slate-400 text-center">Snap a picture of the receipt</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-[11px] text-slate-400 text-center">Choose from gallery</span>
              </button>
            </div>
            <input
              ref={fileInputRef}
        ...
        {step === 'processing' && (
          <div className="text-center py-8 space-y-4">
            ...
            <div className="flex items-center justify-center gap-2 text-base text-blue-400">
            ...
            <p className="text-[11px] text-slate-500">{progress}% — text recognition in progress</p>
          </div>
        )}
              ...
              <div className="border border-slate-800 rounded-xl overflow-hidden">
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    Raw OCR Text
                    <span className="text-[11px] text-slate-500">({rawText.length} chars)</span>
                  </span>
                  {showRawText ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
                </button>
                {showRawText && (
                  <pre className="px-4 pb-3 text-[11px] text-slate-400 font-mono whitespace-pre-wrap max-h-32 overflow-y-auto leading-relaxed">
                    {rawText}
                  </pre>
                )}
              </div>
            ...
            {parsed && (
              <div className="p-3 rounded-xl bg-slate-950/40 border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  {parsed.shopName && <><div className="text-slate-400">Shop:</div><div className="text-white truncate">{parsed.shopName}</div></>}
                  {parsed.matchedKeywords.length > 0 && (
                    <div className="col-span-2 flex flex-wrap gap-1 mt-1">
                      {parsed.matchedKeywords.map((kw, i) => (
                        <span key={i} className="text-[11px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-300">{kw}</span>
                      ))}
                    </div>
                  )}
                </div>
